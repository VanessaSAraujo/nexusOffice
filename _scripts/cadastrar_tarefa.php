...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt = $conexao->prepare($query)) {
        $stmt->bind_param("ssssi", $nome_tarefa, $data_tarefa, $descricao, $mensagem_alerta, $usuario_responsavel);
        ...
        if ($stmt->execute()) {
            ...
            <script language='javascript'>
                swal.fire({
                    ...
                    if (okay) {
                        window.location.href = '../sDashboard.php?r=tarefas'; // Redireciona para a página de cadastro de tarefas
                    }
                });
            </script>
            <?php
